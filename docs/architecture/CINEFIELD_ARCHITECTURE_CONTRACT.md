...
- **Phase 16-C reads two more canonical truths — financial and media — without becoming a second authority for either.** Official scope: Billing/Credits + Assets/Storage/Moderation. Done criterion: admin can inspect financial state and media/storage/moderation state from the same Operations Center, without creating a second billing or media authority. `billing-admin-service.ts` is the FIRST TypeScript reader of Phase 10's `credit_wallets`/`credit_ledger`/`credit_reservations` (plus the `credit_reconciliation` view) — `src/lib/credits.ts` is write/reserve-only and its own header says "NOT WIRED IN YET." Four independent fallible reads combine into `NO_ACCOUNT`/`LEDGER_UNAVAILABLE`/`PARTIAL_DATA`/`FOUND`, the same defensive discipline every other 16-A/16-B admin service uses; no `.rpc(` call exists anywhere in the file — `reserve_credits`/`settle_reservation`/`refund_reservation`/`grant_credits` remain Phase 10's own mutation surface. Phase 15-B's cost ESTIMATE (`cost-contract.ts`: "THIS IS AN ESTIMATE. IT IS NOT SPEND, AND IT IS NOT MONEY OWED") is deliberately never merged into this view, not even side by side — a structural test bans `credit_price_per_unit` reads and any `finops/cost-contract` import, mirroring Phase 15-B's own ban in the other direction. Stripe has zero integration in this repository (no dependency, no webhook, no customer table); the panel renders the static `STRIPE_INTEGRATION_STATE = "STRIPE_NOT_CONFIGURED"`, never a live probe. `asset-admin-service.ts` is the ONE new read of `public.media_assets` (Phase 9), reused by BOTH the Assets/Storage and Moderation pages rather than two competing reads — `bucket`/`object_key`/`backup_bucket`/`backup_key`/`backup_version_id`/`backup_etag` (storage locations/credentials-adjacent), `declared_content_type`/`original_filename` (untrusted, browser/provider-declared), and `legal_hold`/`retention_policy`/`data_class` (Phase 23 hooks with no real values yet) are all deliberately excluded from the column list and the view; `storageBackend` (live/delivery, R2-canonical) and the `backup*` fields (DR/S3-only evidence) are kept as clearly separate fields so a reader can never mistake DR backup completion for the asset being live from S3. `moderation-admin-service.ts` wraps Phase 9-E's `requestMediaRelease`/`approveMediaRelease`/`rejectMediaAsset`/`readSafetyAudit` (`src/lib/media/quarantine-release.ts`) UNMODIFIED — that file already has real, database-enforced two-person approval, `assertRouteAdmin` gating (the same separate `ROUTE_ADMIN_CLERK_USER_IDS` authority `router-admin-service.ts` already distinguishes from `requireAdminAccess()`), and a Phase 12-E policy gate called INSIDE those three functions; this file adds only input validation and a `FORBIDDEN`→`ROUTE_AUTHORITY_DENIED` catch, the same pattern `router-admin-service.ts` established. No new authorization layer, no new policy-registry entry (`policies/data/actions.json` unchanged, verified structurally), no new two-person mechanism — this is "expose the smallest safe admin action," not Phase 16-E's dual-control/passkey territory. Unlike 16-B's AWS/Redis-B-dependent mutations, this action is Supabase-RPC-based and fully testable offline via `FakeSupabaseClient`'s existing `.rpc()` support — `QUARANTINE_RELEASE` is `PASS`, not `DEFERRED_EXTERNAL`. No FinOps display, no live Stripe check, no new mutation beyond the already-built release action, no durable admin-action audit table (same named gap as 16-B). **Phase 16-C does not start 16-D, and does not implement 16-E early.**

- **Phase 16-D adds incident-response visibility over five more canonical owners, plus exactly one guarded action, without becoming a second copy of any of them.** Official scope: Temporal inspect/cancel, Security Center, Incidents/Audit, SLO/Cost Guard, Deploy/Restore Health. Done criterion: during an incident, the operator can see the relevant operational timeline, health, safety and control evidence from one Phase 16 admin surface and can perform only those actions that already have a canonical owner and safe authorization path. Five screens, matching that grouping exactly: `/admin/temporal`, `/admin/security-center`, `/admin/incidents`, `/admin/slo-cost`, `/admin/deploy-restore` — `FUTURE_SECTIONS` (the `layout.tsx` placeholder array carried since 16/1) is now empty and removed; every screen named across 16-A/16-B/16-C/16-D is a real link. **Temporal inspect** (`src/lib/temporal/workflow-inspection.ts`) is the first caller of `WorkflowHandle.describe()` anywhere in this repository — the same cached `getTemporalClient()` and the same deterministic `generationWorkflowId()` every existing Temporal caller already uses, projecting a fixed allow-list (workflow/run id, status name, three timestamps, task queue, a history LENGTH count) and never calling `fetchHistory()`/`result()`, so no raw Temporal payload is reachable. **Temporal cancel** is the ONE mutation this whole package allows: `performAdminTemporalCancel` wraps the UNCHANGED Phase 6R-H `recordCancelIntent`/`requestGenerationCancellation` pair — the same "wrap an existing canonical mutation with fresh server auth, explicit reason, a current-state re-read, fail closed" pattern Phase 9-E's quarantine release established — with one small, additive, backward-compatible extension to `CancelIntentRecord`/`recordCancelIntent` (an optional `adminActorId` that is recorded alongside the reason, never used to relax the existing ownership check: the admin path still passes the REAL owner id it looked up fresh, exactly like the ordinary user-facing route). **Security Center** and **Incidents/Audit** read `security_events` (Phase 12-C, same bounded columns 16-A's own Risk Investigation already established, just queried most-recent-first instead of by identifier) plus the alert router's own live state — `alert-router.ts` gained `listTrackedAlerts()`, a bounded read view of the SAME in-memory dedupe map `raiseAlert`/`resolveAlert` already maintained (two new fields, `type`/`resource`, kept on the existing entry rather than parsed back out of the dedupe key), honestly labelled ephemeral (`alertHistoryPersisted: false`) because it is one process's memory, cleared on every restart, never durably queryable — matching the spec's `ALERT_HISTORY_EPHEMERAL` semantic rather than building a second alert-history store. Incidents/Audit's audit trail combines that live snapshot with two ALREADY-DURABLE stores — `security_events`' `policy_decision_allowed`/`policy_decision_denied` rows (Phase 12-E's own decision log) and `media_safety_audit` (Phase 9-E's release/reject trail) — no new `admin_actions` table, the 16-E handoff named rather than built early. **SLO/Cost Guard**: `slo-cost-admin-service.ts` is the first production caller anywhere in this repository of Phase 15-A's `evaluateSlo()` — six of the seven SLIs are fed real observations (`generation_attempts` for the three generation SLIs, merged Phase 13 `readiness()` reports for `dependency_readiness`, the existing `realtime_outbox_debt()` RPC for `realtime_debt_age`, Phase 7's provider health store for `provider_reliability`); `app_availability` is honestly a single current sample, deliberately below the SLI's own `minimumSamples: 10` (this repository persists no rolling readiness history, and adding one would be exactly the "no migration by default" this batch avoids) — `evaluateSlo` reaches `INSUFFICIENT_DATA` on its own, nothing here special-cases it. Cost Guard calls Phase 15-B's own `runEstimatedSpendGuard()` verbatim. **Deploy/Restore Health**: the rollback signal calls Phase 14-D's `evaluateRollback()` UNCHANGED, fed real Phase 13 readiness and real Phase 13-D open-CRITICAL-alert types, with deployment identity honestly `null` (no deployment-identity tracking exists in this repository — `evaluateRollback` already treats that as a first-class input, never fabricated). Full deploy ELIGIBILITY (`evaluateDeployment`, which needs a specific CI/artifact candidate) is deliberately never called — this repository has no live GitHub Actions/CI, Vercel, or artifact-provenance integration to construct one honestly, and reported as `DEPLOY_EVIDENCE_UNAVAILABLE` rather than fed fabricated inputs just to get a verdict. Restore reuses Phase 15-C's `runRestoreVerification()` verbatim (proves at most a local pg_dump/pg_restore round trip, never live Supabase PITR). Recovery/RTO-RPO reports `RTO_TARGETS`/`RPO_TARGETS` (Phase 15-D/2) at their real size — both `{}` — rather than inventing a target or calling `measureRecovery` with fabricated incident evidence. No second workflow engine, incident router, SLO calculator, cost guard, deploy evaluator, or restore validator exists anywhere in the package — proven structurally, and a closure test (`phase-16d-closure-end-to-end.e2e.test.ts`) walks one realistic incident end to end through the real services: a security event fires → Security Center shows it → the alert router surfaces it as an open incident → the affected generation is inspected through Temporal → the operator cancels it with a reason → the durable cancel intent attributes to the admin → the rollback signal reflects the same open alert. **Phase 16-D does not start 16-E.**

- **Phase 16-E hardens Tier-0 privileged admin authority, additively, without becoming a second identity/policy/queue/routing/billing authority.** Official scope: role separation, step-up assurance, dual control, durable admin-action audit — the exact gap 16-B/16-C/16-D each named by handoff text rather than built early. `requireAdminAccess()`/`decideAdminAccess()` (16/1) are UNCHANGED; `src/lib/admin/admin-privilege.ts` adds a NEW, additive `resolveAdminPrivilegeRole()` (`viewer`/`operator`/`tier0_admin`) from three independent env-var allowlists — `CINEFIELD_ADMIN_CLERK_USER_IDS` now means `viewer` only, `CINEFIELD_ADMIN_OPERATOR_CLERK_USER_IDS`/`CINEFIELD_ADMIN_TIER0_CLERK_USER_IDS` are new and narrower. `src/lib/admin/tier0-action-catalogue.ts` is a pure, zero-import data module classifying every Phase 16 mutation by risk (`READ_ONLY`/`OPERATOR_MUTATION`/`HIGH_RISK_TIER0`) — it never calls an action owner. `src/lib/admin/step-up-auth.ts` (Clerk-free, mirrors `admin-auth.ts`'s split) reads a documented `cinefieldStepUp` session claim and is honestly `NOT_CONFIGURED` for every request today (no Clerk MFA/passkey configuration exists in this repository, and this phase is not permitted to provision one) — `src/lib/admin/require-step-up.ts` is the one documented exception to 16-A's "no `sessionClaims` anywhere" sweep. Dual control reuses one generic mechanism: a new, narrowly-scoped, append-only migration (`20260829000000_tier0_admin_action_audit.sql`, `admin_privileged_action_events`) where an `'approved'` event IS the approval and the threshold is `COUNT(DISTINCT actor_clerk_user_id)`, with self-approval structurally blocked inside `record_admin_privileged_action_approval()` — mirroring, never replacing, Phase 9-E's own quarantine-release two-person mechanism, which stays completely untouched. The SAME table is the durable admin-action audit `security_events` cannot honestly hold (different taxonomy, no request-lifecycle concept) — every lifecycle transition is its own immutable row, never a mutable "current state" update, queryable through the new bounded `GET /api/admin/privileged-audit` screen. `src/lib/admin/tier0-authorization.ts`'s `authorizeTier0Action()` is the decision point; it is wired (additively, never replacing the canonical owner) into DLQ redrive, route disable, and Temporal cancel, and ALWAYS records the real decision. **SECURITY FIX BATCH correction, replacing the original claim in this paragraph:** the original version let `CINEFIELD_TIER0_ENFORCEMENT_MODE` turn a real DENY into `allowed: true` whenever the env var was not exactly `"enforce"` — the default, unset state — which a post-implementation closure audit proved let a normal admin session (no Tier-0 role, no step-up) execute all three wired Tier-0 actions, a real, live-traced violation of the phase's own binding invariant (audit verdict: `PHASE_16_E_FINAL_STATUS: FAIL`, `TIER0_SHADOW_MODE: FAIL_OPEN`). The fix removed that lever: `allowed` is now the authorization truth in every mode, unconditionally; `CINEFIELD_TIER0_ENFORCEMENT_MODE` is parsed only for an `enforcementMode` label stamped onto the decision for telemetry, and every value other than the literal `"enforce"` (missing, malformed, or explicit `"shadow"`) is safe now precisely because `"shadow"` no longer bypasses anything. Practical effect: every `HIGH_RISK_TIER0` action that requires step-up is refused for every caller until Clerk MFA/passkey is externally configured — correct fail-closed behavior, not a regression. The same fix batch added `src/lib/security/privileged-mutation-guard.ts` (`guardPrivilegedMutation()`), the one canonical, identity-free, repository-owned Origin+Content-Type guard now called first in every privileged mutation route (DLQ redrive, route disable, Temporal cancel, quarantine release, BullMQ retry) — closing a real gap the closure audit also found: the previous "no new CSRF mechanism" claim relied only on implicit Clerk cookie behavior and an informal CORS-preflight assumption that a `text/plain`-declared, JSON-bodied cross-site request could defeat. No OPA runtime, no break-glass, no second rate limiter, no AI/MCP reachability — all still verified structurally. **Phase 16-E does not start Phase 17.**

- **Phase 17 adds a semantic layer that decides WHAT to generate; it never decides HOW — five official packages (17-A–17-E) per the authoritative master roadmap (v1.9.1), reconciled against repository reality package by package.** Official scope: `UserIntent` → Core Coordinator → Specialist Directors → Manifest Compiler → `GenerationManifest`, under `src/lib/product-intelligence/`. Temporal remains the ONLY generation lifecycle owner — pinned structurally: no file in the package imports Temporal, the SQS/BullMQ transport, or a `*-provider.ts` adapter. `model-registry.ts`/`capability-validator.ts` (`ALREADY_EXISTS`, narrow scope) and `classifyAndEnhancePrompt()`/`moderateText()` (existing, already-shipped Cloudflare Workers AI specialists) are reused rather than duplicated. **Ownership correction, this batch:** the Control/Panel Schema Registry belongs to Phase 7, not Phase 17 — the roadmap places it there explicitly. It is still `MISSING` repository-wide (four+ independent, non-shared per-studio control catalogs — `cinemaStudioData.ts`, `imageModelCapabilities.ts`/`createImageData.ts`, `voiceoverModelConfig.ts`, Marketing Studio's inline constants — is a real, pre-existing, un-touched-by-Phase-17 duplication risk, named here as a Phase 7 handoff, not fixed here), so Phase 17 still does not fabricate structured, capability-validated fields for camera/lighting/genre/era/tempo/palette — those stay bounded, optional free-text `styleHints` composed into the prompt. Studio Profile / creative-preset intelligence is honestly `PARTIAL`, not complete: `styleHints` is free text indistinguishable from prompt content, not real profile intelligence, and a genuine profile system cannot be built honestly without the Phase 7 Control/Panel Schema Registry existing first to give a profile something real to reference — recorded as a `FUTURE_OWNER_HANDOFF`, not faked.

  **17-A — AI Director + model-specific Prompt Compiler + multilingual intelligence + Workflow/Skill Engine. Done: one intent becomes a model-aware executable generation plan. STATUS: complete.** "AI Director" is satisfied by `core-coordinator.ts`'s `coordinateUserIntent()` — deliberately not renamed to "Director" (would collide with Cinema Studio's own UI "Director Panel" naming); a repo-wide search confirmed no other backend "Director" concept exists to reconcile against. The model-specific Prompt Compiler is a real hardening this batch: `manifest-compiler.ts`'s `composePrompt()` now re-validates the FINAL composed string (raw prompt + `styleHints`) against the target model's own real, registry-owned `capabilities.maxPromptLength`, refusing (`UNSUPPORTED_CAPABILITY`) rather than silently truncating — closing a real gap where `styleHints`, appended AFTER the Capability Specialist's own prompt-length check, could previously push a compiled manifest's prompt past the very limit that check had just passed. Multilingual intelligence stays honest: `settings.language` is real and capability-validated against `model.capabilities.supportedLanguages`; raw multilingual text passes through unmodified, no auto-translation is performed — detection/translation was explicitly deferred by an earlier roadmap phase ("must not silently start translating or rewriting user text") and stays deferred. The Workflow/Skill Engine requirement is reconciled, not rebuilt: no generic "skill/workflow" runtime exists anywhere in the repo, but `UserIntent` (prompt + references + settings + `styleHints`) already IS the bounded "what steps/semantic requirements a product flow needs" description the roadmap's own definition calls for, and, per that same definition, already owns none of Temporal/SQS/provider execution/credit settlement — a second, parallel type describing the same thing would be exactly the duplicate-concept-creation this whole layer exists to avoid. **Real admission integration, closing the previous audit's "compatibility-seam-only" finding:** `POST /api/product-intelligence/execute` is a new, real, live HTTP path — `UserIntent` → Coordinator/Specialists → `GenerationManifest`, and ONLY on a clean `VALID` compile, straight into `createGeneration()` then `respondToGenerationRequest()`, the exact same two canonical functions `/api/generate/route.ts` calls, in the same order, imported and called verbatim — no reimplementation, no second lifecycle. Any non-`VALID` outcome (including `PARTIAL`, e.g. safety unavailable) stops there, unexecuted — this is the "plan ≠ execution" gate: a caller previews with `/compile`, and only a separate, deliberate `/execute` call can actually create work. `createGeneration()`'s own `create_generation_tx` RPC already verifies project ownership server-side (a foreign `projectId` surfaces as not-found) — reused verbatim, so this path inherits real tenant/project authorization automatically, without reimplementing it. `/api/generate/route.ts` itself is completely unmodified.

  **17-B — Cinema Studio + Marketing Studio + Soul/Character + Product workflows share one common generation core. Done: four product flows use the same core. STATUS: PARTIAL — the shared core is real where flows actually reach it, but not all four names resolve to independently-wired flows.** Cinema Studio (LOCKED, untouched) already reaches `createGeneration()`/`/api/generate` through its own `useGeneration()` hook — the same canonical core the new `/execute` route also calls. Marketing Studio is a real, functioning UI (`src/components/marketing-studio/MarketingStudioProductWorkspace.tsx`) whose default, visible target/mode/style catalog currently falls through to a `console.log` placeholder rather than a real call — only an internal `?model=` dev override reaches the same canonical path Cinema Studio uses; not fixed this batch, since wiring the visible catalog to real generation is a distinct, unauthorized UI change outside Phase 17's remit. "Soul/Character" does not exist as a separate product flow — it is a capability toggle (`SoulGeneralCard.tsx`/`CharacterCardPopover.tsx`) nested inside `/image`'s `ImageForm.tsx`; classified `NOT_APPLICABLE` as a fifth flow, not `MISSING`, since the roadmap name maps onto an existing feature rather than an absent product. "Product" is `AMBIGUOUS_NAME` — it resolves to Marketing Studio's own `/product` page; no separate flow exists. No locked or unlocked product UI was modified to close this gap.

  **17-C — Virality analysis + Cinefield Supercomputer planning/cost/approval/execution/quality-control flow. Done: plan becomes safe execution only after user approval. STATUS: DEFERRED_FUTURE.** Virality analysis does not exist anywhere — "Viral Presets" is a marketing gallery label (`HeroCarousel.tsx`) with zero scoring/analysis logic behind it. `/supercomputer` (LOCKED, untouched) is a UI shell whose composer reaches `/api/generate` only through a dev-only `?model=` override; its visible model/category UI has no click handlers, and no planning, cost-estimation, approval, or quality-control concept is connected to it. Real, reusable infrastructure a future 17-C batch could build on already exists and is unmodified: Phase 15-B's cost estimator (`src/lib/finops/cost-contract.ts`, real, `ESTIMATE_BASED` only, never real spend) and Phase 16-E's step-up/elevated-session mechanism (admin-scoped only today, not an end-user "confirm before spend" gate). Nothing was built this batch — inventing virality-analysis logic with no product spec, or editing the LOCKED Supercomputer page, are both out of Phase 17's authorized scope without explicit unlock.

  **17-D — MCP + Apps Builder + Websites Builder + Games Builder share the same generation core, auth boundary, and billing boundary. Done: external clients generate without provider secret/routing exposure. STATUS: DEFERRED_FUTURE / NOT_APPLICABLE.** None of the four exist in this repository under any name, in code or docs — this repository's own section 31 already says "no MCP server exists... target architecture, not implemented," and Phase 16-E's `AI_REACHABLE_TIER0_ACTION = NO` structural test already proves (by scanning for and finding zero MCP/AI-agent-facing files) that nothing reaches the admin boundary this way. Apps/Websites/Games Builder have zero code and zero doc mentions anywhere. No placeholder builder was created "to check a roadmap box," per this batch's own explicit instruction.

  **17-E — Model/prompt version changes gated by the Phase 22 eval regression gate + Braintrust MCP read/analysis. Done: no default route/prompt promotion without quality measurement. STATUS: DEFERRED_TO_PHASE_22.** Phase 22, an eval regression gate, and Braintrust are entirely unbuilt — confirmed by direct repository search. The only real artifact is a forward-declared landing pad Phase 7's own routing code already owns: `src/lib/routing/route-quality.ts`/`routing-policy.ts` declare quality weight as "EMPTY TODAY, on purpose... declared so Phase 22 has a place to land, not so it can do something now," and `phase-7d-cost-quality-routing.e2e.test.ts` pins that no quality column or seed may exist before Phase 22. Nothing was implemented this batch, per explicit instruction not to build Phase 22 prematurely. `manifest-compiler.ts`'s new `MANIFEST_COMPILER_VERSION` constant (distinct from `GENERATION_MANIFEST_VERSION` — one versions the compiler's decision logic, the other the manifest's field shape; governed the same way `security-event-logger.ts`'s `policy_version` already is, by hand-bumping on real logic change) is a real, separate governance track Phase 22 could eventually gate promotion against once it exists — that wiring remains Phase 22's to build, not claimed done here.

  `GenerationManifest` is versioned (`manifestVersion`, `compilerVersion`, both `"1.0.0"`) and provider-neutral by construction — no `fal.*`/`runway.*`/`openai.*`/`xai.*`/`providerJobId`/`providerUrl`, pinned structurally — and excludes provider, credentials, queue receipt, Temporal state, retry counter, billing settlement state, and raw provider response by design. Persistence decision, unchanged: EPHEMERAL for this batch — a manifest is computed on demand and returned, never written to a new table or folded into `generations.metadata` (already claimed by `cancel-intent.ts` for an unrelated concern); `MANIFEST_PERSISTENCE_DECISION_REQUIRED` was not triggered. **Phase 17 does not start Phase 18.**
