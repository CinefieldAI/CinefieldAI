...
- **A stored object is not verified media.** Phase 9-A stores opaque bytes and records what the provider or the browser DECLARED. `verified_mime`, `checksum_sha256`, `moderation_status` and `quarantine_status` exist on `media_assets` as hooks defaulted to not-yet-done — filling them is Phase 9-B, which also owns the untrusted-parser sandbox. No decoder, sniffer or transcoder may enter the media path before that sandbox exists.
