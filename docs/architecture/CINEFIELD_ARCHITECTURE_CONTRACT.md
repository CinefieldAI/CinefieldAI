...
- **Realtime has one canonical path, and only its first arrow exists (Phase 11).** `Postgres Outbox → Notification Service → Redis Streams → SSE Gateway → Browser`. The roadmap's binding correction closed the "Pub/Sub veya Streams" option — Streams, because Pub/Sub provides neither cursor nor replay — and forbids a direct realtime publish from Worker, Media or Provider. **11-A implemented the Notification Service and nothing downstream of it**: no module imports a Redis client, no SSE surface exists, and the tests assert that absence rather than trusting it. Redis Streams will live on **Redis A** (application state); Redis B stays BullMQ-queue-state-only, and realtime streams must never be merged into it.
