...
- **Phase 17 adds a semantic layer that decides WHAT to generate; it never decides HOW.** Official scope: `UserIntent` → Core Coordinator → Specialist Directors → Manifest Compiler → `GenerationManifest`, under `src/lib/product-intelligence/`. Temporal remains the ONLY generation lifecycle owner — pinned structurally: no file in the package imports Temporal, the SQS/BullMQ transport, or a `*-provider.ts` adapter. A pre-implementation reality audit (full-repo vocabulary sweep) confirmed the layer itself is genuinely greenfield, while `model-registry.ts`/`capability-validator.ts` (`ALREADY_EXISTS`, narrow scope) and `classifyAndEnhancePrompt()`/`moderateText()` (existing, already-shipped Cloudflare Workers AI specialists) are reused rather than duplicated. It also confirmed a Control/Panel Schema Registry and a Studio Profile/preset system are both `MISSING` — so Phase 17 does not fabricate structured, capability-validated fields for camera/lighting/genre/era/tempo/palette; those stay bounded, optional free-text `styleHints` composed into the prompt, never a validated control. `capability-specialist.ts` is the one deterministic, non-AI specialist and calls `findModel()`/`resolveWorkflow()`/`validateCapabilities()` directly — never a second capability source. `manifest-compiler.ts` is deterministic (no provider selection, no workflow start, no credit reserve, no terminal-state write) and applies the brief's exact precedence: safety/policy > capability > explicit user choice > specialist suggestion > default (the profile tier is skipped, not fabricated). `GenerationManifest` is versioned (`manifestVersion: "1.0.0"`) and provider-neutral by construction — no `fal.*`/`runway.*`/`openai.*`/`xai.*`/`providerJobId`/`providerUrl`, pinned structurally — and excludes provider, credentials, queue receipt, Temporal state, retry counter, billing settlement state, and raw provider response by design. `compatibility-seam.ts` proves, with pure mappers, that a compiled manifest CAN feed the existing `generation-create-service.ts`/`generation-settings-mapper.ts` seam, but nothing is wired automatically — `/api/generate` is untouched, and there remains exactly one canonical generation owner. The new `POST /api/product-intelligence/compile` route only compiles and returns a manifest; it never creates a `generations` row, starts Temporal, or reserves credits, and reaches `guardRoute({ routeClass: "paid_compute" })` before its body is even parsed, the same shape as `/api/orchestration/enhance-prompt`/`moderate-text`. Persistence decision: EPHEMERAL for this batch — a manifest is computed on demand and returned, never written to a new table or folded into `generations.metadata` (already claimed by `cancel-intent.ts` for an unrelated concern); `MANIFEST_PERSISTENCE_DECISION_REQUIRED` was not triggered, this is a reasoned choice. **Phase 17 does not start Phase 18.**
