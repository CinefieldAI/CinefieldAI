...
- **AI/MCP write authority is OFF by default and cannot be reached by borrowing a role.** Roadmap ¶1856: MCP connections start READ-ONLY, and a write goes AI suggestion → policy → human approval → protected workflow, never skipped for incident urgency. The allowlist is empty, the agent check runs BEFORE the role check, and the origin check runs before both — so a browser-driven agent is refused as a browser. Nothing calls the AI entry point yet: the guardrail landed before the capability, which is the order that makes it real.
...
This is the health *foundation* only. **Phase 13-C (external uptime, synthetic checks, heartbeat and public status) is NOT complete** — no external monitor is configured, nothing calls `/api/health/live` from outside, no worker pushes a heartbeat, and no alert fires when a runtime goes `UNREADY`. When 13-C needs more than liveness from outside, the answer is an authenticated internal route or a push heartbeat decided then — not this endpoint quietly growing a `?verbose=1`.
