...
- **The canonical user owner is `profiles`, and its `service_role` grant is asymmetric (16-A/3).** `public.profiles` (`clerk_user_id` primary key) is the one user-identity table in this schema; `generations.clerk_user_id` and `projects.clerk_user_id` both carry `REFERENCES profiles(clerk_user_id) ON DELETE CASCADE`. No workspace table exists at all — "workspace" means `projects` here, surfaced only as the same bounded `projectId` reference 16-A/2 already exposes, never expanded into its own screen. `getAdminUserInvestigation()` (`user-investigation-service.ts`) runs two narrow, explicit-column reads — `profiles` (one row, by `clerk_user_id`) and `generations` (≤20 rows, by the same `clerk_user_id` ownership column 16-A/2 already trusts, `created_at desc`) — but runs them as two INDEPENDENT fallible reads rather than one shared `Promise.all` + one catch, because the committed grants in `20260805132704_remote_schema.sql` are not symmetric: `generations` has `GRANT ALL ... TO service_role` and `projects` has an explicit `SELECT` grant, but `profiles` grants `service_role` only `REFERENCES,TRIGGER,TRUNCATE,MAINTAIN` — no `SELECT`. This batch cannot connect to production Postgres to confirm whether that is still the live grant, and adding a migration to fix it is out of scope for a read-only slice, so the service is written to degrade honestly if it is: `PARTIAL_DATA` (a state the client-state layer keeps distinct from `FOUND`/`NO_GENERATIONS`/`EVIDENCE_UNAVAILABLE`) names exactly "we have real generation evidence for this identity but the profile enrichment could not be read," never silently discarding the generation evidence or fabricating a profile. `EVIDENCE_UNAVAILABLE` covers the case where neither read can confirm existence either way. Same `requireAdminAccess()` boundary and opaque-404 convention as 16/1 and 16-A/2. `email`/`username`/`avatar_url` and the billing-adjacent `credits`/`plan` are deliberately never selected — the latter two sit on the same Phase 10 economic-truth boundary 16-A/2 already drew around `cost_amount`/`cost_currency`. Each recent-generation row links to `/admin/generations?generationId=<id>` by id — the existing 16-A/2 surface is reused, never re-implemented (a structural test asserts the new files never re-declare `buildTraceChain`/`TraceChainLink` or re-query `outbox_events`/`generation_attempts`/`media_assets`). Workspaces and Risk remain unbuilt.
