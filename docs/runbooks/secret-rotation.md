...
| `CINEFIELD_SECRETS_ANOMALY_INGEST_TOKEN` | ⚠️ cut-over | Phase 25-D. Same shape as the drift-ingest token above — one shared bearer secret, one intended caller (a live EventBridge/Lambda forwarder, not yet wired). Rotate identically: generate a new value, update both sides together, confirm the next real forwarded event authenticates. |
| `CINEFIELD_PROVENANCE_SIGNING_KEY_PEM` | ✅ dual-key overlap | Phase 27-B. **Overlap is mandatory here, not a nicety.** Every `media_provenance` row already signed by the outgoing key must keep verifying after rotation, so the old PUBLIC key stays in the trusted-key map indefinitely while only new output is signed with the new private key. Procedure: generate a new ES256 keypair **outside the repository**, publish the new public key under a new `CINEFIELD_PROVENANCE_SIGNING_KEY_ID`, deploy the new private key, confirm freshly recorded evidence verifies under the new id, then retire the old PRIVATE key only — never the old public key, or historical evidence becomes `UNTRUSTED_SIGNER`. This is the one credential in this table whose "revoke" step is deliberately asymmetric. |
...
| Provider/internal secrets held in AWS Secrets Manager (once `AwsSecretsManagerProvider` is installed, Phase 25-B/C) | ✅ managed | `docs/runbooks/secret-rotation.md`'s manual procedure above still applies to whatever is NOT yet moved to Secrets Manager. Once installed, `src/lib/secrets/rotation-execution-service.ts`'s `executeSecretRotation()` automates create-new-version -> verify -> ACTIVE through the same two-person `secret.rotate` gate this table's other cut-over/dual entries still require an operator to drive by hand — see that module's own header for the authority chain. |
