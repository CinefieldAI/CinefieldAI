...
export default function GeminiAspectRatioControl({
  ...
  const nav = useListboxNav({
    count: GEMINI_ASPECT_RATIOS.length,
    selectedIndex: GEMINI_ASPECT_RATIOS.findIndex((opt) => opt.value === value),
    open: controlledOpen,
    onSelect: (index) => {
      const opt = GEMINI_ASPECT_RATIOS[index];
      if (!opt) return;
      onChange(opt.value);
    ...
    <Popover.Root open={controlledOpen} onOpenChange={handleOpenChange}>
      ...
      <Popover.Portal container={portalContainer}>
        ...
        >
          ...
          {GEMINI_ASPECT_RATIOS.map((opt, index) => {
