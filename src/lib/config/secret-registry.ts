...
export const SECRET_REGISTRY: readonly SecretEntry[] = [
  ...
  { name: "CINEFIELD_PROMPT_MODERATION_ENGINE", class: "IDENTIFIER_NON_SECRET", requirement: "OPTIONAL", group: "runtime", rotation: "NOT_A_SECRET", note: "Phase 28-A. Names the prompt moderation engine; not a credential. The registry is empty, so any value resolves to no engine rather than to a permissive fallback." },
  { name: "CINEFIELD_REFERENCE_INPUT_EVALUATOR", class: "IDENTIFIER_NON_SECRET", requirement: "OPTIONAL", group: "runtime", rotation: "NOT_A_SECRET", note: "Phase 28-A. Names the reference-image (face/NCII/deepfake) evaluator; not a credential. The registry is empty, so any value resolves to no evaluator rather than to a permissive fallback — and with none configured a request carrying a reference image is REFUSED, not admitted unexamined." },
  { name: "CINEFIELD_REPEAT_VIOLATION_THRESHOLD", class: "IDENTIFIER_NON_SECRET", requirement: "OPTIONAL", group: "runtime", rotation: "NOT_A_SECRET", note: "Phase 28-C. How many blocks make a repeat offender. UNSET by design — the roadmap names no number, and one invented here would be the number that closes a customer's account. Unset means enforcement reports BUSINESS_DECISION_REQUIRED." },
  { name: "CINEFIELD_REPEAT_VIOLATION_WINDOW_DAYS", class: "IDENTIFIER_NON_SECRET", requirement: "OPTIONAL", group: "runtime", rotation: "NOT_A_SECRET", note: "Phase 28-C. The window the threshold above is counted over. Unset for the same reason." },
