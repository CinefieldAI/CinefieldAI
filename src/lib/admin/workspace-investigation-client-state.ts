import type { WorkspaceInvestigationResult } from "./workspace-investigation-contract";

/**
 * Pure response -> UI-state mapping for the Phase 16-A/4 Workspace
 * Investigation panel.
 *
 * Kept out of the `"use client"` component, the same split
 * `user-investigation-client-state.ts` (Phase 16-A/3) already established,
 * so it is testable with `node:test` without a DOM.
 *
 ...
 * denial vs. the full `{outcome:"WORKSPACE_NOT_FOUND"}` result for a genuine
 ...
 */
export type WorkspaceInvestigationPanelState =
  | { readonly kind: "idle" }
  | { readonly kind: "loading" }
  | { readonly kind: "denied" }
  | { readonly kind: "not_found" }
  | { readonly kind: "unavailable"; readonly reasonCode: string }
  | { readonly kind: "found"; readonly result: Extract<WorkspaceInvestigationResult, { outcome: "FOUND" }> }
  | {
      readonly kind: "no_generations";
      readonly result: Extract<WorkspaceInvestigationResult, { outcome: "NO_GENERATIONS" }>;
    }
  | {
      readonly kind: "partial_data";
      readonly result: Extract<WorkspaceInvestigationResult, { outcome: "PARTIAL_DATA" }>;
    };
...
function isResultShape(value: unknown): value is WorkspaceInvestigationResult {
...
export function interpretWorkspaceInvestigationResponse(
  status: number,
  body: unknown
): WorkspaceInvestigationPanelState {
  if (status === 404) {
    if (isResultShape(body) && body.outcome === "WORKSPACE_NOT_FOUND") {
  ...
  if (body.outcome === "WORKSPACE_NOT_FOUND") {
    return { kind: "not_found" };
  }
...
export function networkErrorState(): WorkspaceInvestigationPanelState {
