import type { DeployRestoreAdminResult } from "./deploy-restore-admin-contract";

export type DeployRestorePanelState =
  | { readonly kind: "loading" }
  | { readonly kind: "denied" }
  | { readonly kind: "unavailable"; readonly reasonCode: string }
  | { readonly kind: "found"; readonly result: Extract<DeployRestoreAdminResult, { outcome: "FOUND" }> };
...
export function interpretDeployRestoreResponse(status: number, body: unknown): DeployRestorePanelState {
